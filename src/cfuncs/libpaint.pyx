...
def drawp(canvas, uint32_t new_color, int radius, int x, int y, int mx, int my):
    x, y = rel_pos(canvas, x, y); mx, my = rel_pos(canvas, mx, my)
    if width != canvas.width or height != canvas.height: resize(canvas.width, canvas.height)
